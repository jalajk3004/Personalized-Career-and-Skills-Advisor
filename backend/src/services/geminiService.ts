...
class GeminiService {
   ...
   */
  private buildFollowUpQuestionsPrompt(previousQA: QuestionAnswer[], numberOfQuestions: number): string {
...
`;
   ...
   */
  private buildRoadmapPrompt(career: string, userData: CareerData, allAnswers?: QuestionAnswer[]): string {
    const userProfile = `
**User Profile:**
- Name: ${userData.name}
- Age: ${userData.age}
...
`;
  ...
  private validateRoadmap(roadmap: any, career: string): CareerRoadmap {
    ...
    const validatedSteps: RoadmapStep[] = [];
    roadmap.roadmap.forEach((step: any) => {
